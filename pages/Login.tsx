...
function getAuthErrorMessage(params: URLSearchParams): string | null {
  ...
  if (error === 'access_denied') {
    return "Acces refuse. Vous n'avez pas autorise la connexion Google.";
  }
  if (errorCode === 'unexpected_failure' || errorDescription.includes('exchange external code')) {
    return "Echec de la connexion Google. Veuillez reessayer ou utiliser votre email et mot de passe.";
  }
...
const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
      ...
      try {
          ...
          console.error(error);
          setAuthError("Erreur de connexion Google. Veuillez reessayer.");
      }
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-rose-50 to-white px-6">
      <div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl border border-rose-100 text-center">
        ...
        <button
            ...
            className="w-full bg-white border border-slate-200 text-slate-600 font-medium py-2.5 rounded-xl hover:bg-slate-50 transition-all flex items-center justify-center gap-3 text-sm"
        >
